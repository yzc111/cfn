...
function deletemark() {
	...
	} else if (str.match(/<supp_\d+\s\S+\s>|<supp\s\S+\s>/)) {
		var r = confirm("将要删除标注：\n" + str);
		if (r) {
			str = str.replace(/<supp_\d+ /, "");
			str=str.replace(/<supp\s/, "");
			str = str.replace(/ >/, "");
			if (navigator.appName == "Netscape") {// chrome
				range.deleteContents();
				range.insertNode(document.createTextNode(str));
...
function faq(){
	...
	if (islogin) {	
		...
		var link = "./dist2/index.html"+'?value='+"";
...
//	charlescqh
		//
		if(value=='charlescqh')
			{var link = "./upload.jsp"+'?value='+"";}
		else
			{
			var link = "./listfile.jsp"+'?value='+"";
...
//								+ sentencejson.sentenceid + ")'>删除</a>";
								num++;
								});
						document.getElementById("showsentence").innerHTML = sentencestr;
...
//									+ sentencejson.sentenceid + ")'>删除</a>";
//						}
//						document.getElementById("sentencetable").innerHTML = sentencestr;
...
function reback() {
	if(window.confirm("该操作将会清除当前框架标注结果，是否继续？")){
		allsupparray[chooseframeindex] = new Array();
		supparray = allsupparray[chooseframeindex];
		if (chooseframeindex == -1) {
			alert("ERROR!!!");
			return;
		}
		document.getElementById("Separated Sentence").innerHTML = rebacksen[chooseframeindex];
		separatedsenstringarray[chooseframeindex] = rebacksen[chooseframeindex];
	}
}
...
function displayseparated(data) {
	...
	$.each(eval(data.mss), function(i, item) {
		...
		rebacksen.push(item.marksentence);
		lexicualunitsidarray.push(item.lexicalunitid);
		$.each(eval(item.supps),function(i,supp){
			supparray.push(supp);
		});
		allsupparray.push(supparray);
	});
...
function finish() {
	...
	for (var i = 0; i < checkobjs.length; i++) {
		if (checkobjs[i].checked) {
			sentencestrings.push(formatsuppstr(separatedsenstringarray[i]));
...
function login(iscookie) {
	...
	$
			.post(
					"/assianno/servlet/UserLoginServlet",
					{
						...
						if (status == "success") {
							...
							if (returnobj.result == true) {
								if (!iscookie) {
									 ...
									 * 
									 */
//									$('#faq').attr('href','./dist2/index.html'); 
									
									alert("Login Success");
								}
								
								loginpage(returnobj.username,
										returnobj.auditcount);
							...
							document.getElementById("grammarfunction").innerHTML = gfstr;
							document.getElementById("dnidiv").innerHTML = "<input type='radio' name='grammarfunction' value='dni'>有定的零形式";
...
function displayinife() {
	...
	$
			.each(
					...
					function(n, item) {
						var isdni = true;
						for (var i = 0; fes != null && i < fes.length; i++) {
							if(fes[i].indexOf("&lt;"+item.frameelement_en_short+"-")!=-1){
								isdni=false;
								break;
							}
							if(fes[i].indexOf("&lt;"+item.frameelement_en_short+"+")!=-1){
...
function selectsentence() {
	...
	var selecttype = "";
	var selectradio = document.getElementsByName("selecttype");
						...
						if (status == "success") {
							...
							if (frameid == null || frameid == "") {
								...
								for (var i = 0; i < framejsons.length; i++) {
									...
									framestr += "<option value='"
											+ framejson.frameid + "'>"
											+ framejson.framename + "</option>";
								}
								document.getElementById("frameselect").innerHTML = framestr;
							}
...
function auditload() {
	$
			.post(
					"/assianno/servlet/GetUserAuditServlet",
					{},
					function(data, status) {
						if (status == "success") {
							var jsons = eval("(" + data + ")");
											...
											function(n, item) {
												if (item.status == 1)
													document
															...
															+ "</td><td>"
															+ item.sentence
															+ "<a href='./servlet/WCSRedirectServlet?presentenceid="
															+ item.presentenceid
															+ "&willchangesentenceid="
															+ item.sentenceid
															+ "'>修改</a></td><td>"
													...
													document
															.getElementById("audittable").innerHTML += "<tr><td>"
															+ n
															+ "</td><td><font color='#FF0000'>NEW</font>"
															+ item.sentence
															+ "<a href='./servlet/WCSRedirectServlet?presentenceid="
															+ item.presentenceid
															+ "&willchangesentenceid="
